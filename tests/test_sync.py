...
def test_async_send_arp():
    ...
    response = aioarp.request('127.0.0.1', 'null', sock=msocket)
    assert response
...
def test_async_send_arp_timeout():
    with MockSocket(eth_header_no_arp + arp_header) as msocket:
        with pytest.raises(aioarp.NotFoundError):
            aioarp.request('127.0.0.1', 'null',  sock=msocket, timeout=0.5)

@pytest.mark.anyio
def test_invalid_response_ip_mismatch():
    with MockSocket(eth_header_arp + arp_header) as msocket:
        with pytest.raises(aioarp.NotFoundError):
            aioarp.request('127.0.0.2', 'null', sock=msocket, timeout=0.5)
